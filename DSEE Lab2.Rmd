...
MLO_CO2_long <- gather(MLO_CO2_wide,Year, CO2,        # in this order: data frame, key, value
                                      c(3:50))        # we gather CO2 values by year
```
...
<mark style="background-color:#F9E59B">- Whether the value you are calling is a character or numeric value: above, M is of class integer (a number), so we don’t need quotes around the values it takes.</mark>
...
Are these differences statistically significant? We’ll go into more detail regarding how to test this in subsequent labs, but we can run a quick linear regression right now because who doesn’t like a little instant gratification.
